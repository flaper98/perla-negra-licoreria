    ...
    <main className="bg-[#f4efe4]">
      ...
      <section className="mx-auto min-h-screen max-w-7xl px-6 pt-36 pb-16">
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {results.map((product) => (
            <article key={product.name} className="rounded-3xl bg-white p-5 text-center shadow-lg">
              <img src={product.image} alt={product.name} className="mx-auto h-44 object-contain" />
              <p className="mt-4 text-xs font-black uppercase text-yellow-700">{product.category}</p>
              <h3 className="mt-2 font-black">{product.name}</h3>
              <p className="mt-3 font-black text-red-700">{product.unitPrice}</p>
